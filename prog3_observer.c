...
int main( int argc, char **argv) {
	...
	if(can_join == 'Y') {
		...
		while (valid == false) {
			...
			if( valid_username == 'T' || valid_username == 'N') {
				valid_length_and_regex = false;
				printf("Closing the observer connection\n");
				close(sd);
				exit(EXIT_SUCCESS);
			}
		...
		while (1) {
			uint16_t msg_length;
			n = recv(sd, &msg_length, sizeof(msg_length), MSG_WAITALL);
			check_valid_send_and_recv(n);
			msg_length = ntohs(msg_length); /* converts nbo hbo */

			char msg[msg_length +1];
			memset(msg, 0, msg_length+1);

			n = recv(sd, &msg, msg_length, MSG_WAITALL);
			check_valid_send_and_recv(n);
